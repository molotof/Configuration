...
class CustomApplications(Applications):
    def app_default(self, c):
        ...
        if f.extension is not None:
            ...
            if f.extension == 'djvu':
                return self.either(c, 'zathura')
            if f.extension in ('xml', 'csv', 'php', 'js'):
                return self.either(c, 'editor')
            if f.extension in ('html', 'htm', 'xhtml') or f.extension == 'swf':
                c.flags += 'd'
                handler = self.either(c, 'firefox', 'chromium', 'luakit')
            ...
            if f.extension in ('swc', 'smc', 'sfc'):
                return self.either(c, 'snes9x-gtk', 'zsnes')
            if f.extension in ('odt', 'ods', 'odp', 'odf', 'odg', 'doc', \
                   'xls', 'docx', 'pptx', 'xlsx', 'ppt'):
                return self.either(c, 'libreoffice')
            if f.extension == 'jar':
                c.flags += 'd'
                return self.either(c, 'javajar')
            if f.extension == 'xcf':
                return self.either(c, 'gimp')
            if f.extension == 'exe':
                return self.either(c, 'gimp')
            if f.extension in (c, 'mkv', 'm4v', 'm4a', 'vob', '3gp'):
        ...
        if f.image:
            if c.mode in (11, 12, 13, 14):
                ...
                return self.either(c, 'mirage', 'feh')
    ...
    @depends_on('javajar')
    def app_javajar(self, c):
        c.flags += 'd'
        return 'java', '-jar', c.file.path
...
CustomApplications.generic('fceux', deps=['X'])

# Add those which should only run in X AND should be detached/forked here:
CustomApplications.generic(
    'luakit', 'firefox', 'opera', 'chromium', 'zathura', 'mirage', 'gimp', \
    'libreoffice', 'java', 'javac', 'wine', 'snes9x-gtk', \
    flags='d', deps=['X'])
